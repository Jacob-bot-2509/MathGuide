...
**收尾标记是契约的一部分**:每条流(含各降级分支)都以 `data: [DONE]` 结束,
由 `chat.py` 的 `_sse` 统一补发,生成器不必也不能自己 yield(重复的会被丢弃)。
前端据此判断回答是否完整 —— 没等到 `[DONE]` 就断开(后端重启、网络中断)时,
已收到的正文只是半截,界面会标「回答已中断」,不会当成完整回答存进历史。
改后端时若绕过 `_sse` 直接返回 `StreamingResponse`,这条判定就失效了。
